...
def convert_xml(old_xml_str, env_name, env):
    ...
    if env_name in ["PickPlaceCan", "NutAssemblySquare", "ToolHang"]:
        ...
        cube_new = find_elements(
            root=new_et, tags="body", attribs={"name": "cube_main"}, return_first=True
        )

        cube_old = find_elements(
            root=old_et, tags="body", attribs={"name": "cube_main"}, return_first=True
        )

        worldbody_new = find_elements(root=new_et, tags="worldbody", return_first=True)
        ...
        for bname in [
            ...
            body_new = find_elements(
                root=new_et, tags="body", attribs={"name": bname}, return_first=True
            )

            body_old = find_elements(
                root=old_et, tags="body", attribs={"name": bname}, return_first=True
            )
