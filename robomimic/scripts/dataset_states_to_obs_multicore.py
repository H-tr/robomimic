...
def write_traj_to_file(
    args, output_path, total_samples, total_run, processes, is_robosuite_env, mul_queue
):
            ...
            if not mul_queue.empty():
                ...
                try:
                    ...
                    for k in datagen_info:
                        ep_data_grp.create_dataset(
                            "datagen_info/{}".format(k), data=np.array(datagen_info[k])
                        )
                    ...
                    if "ep_info" in f["data/{}".format(ep)].attrs:
                        ep_data_grp.attrs["ep_info"] = f["data/{}".format(ep)].attrs[
                            "ep_info"
                        ]
                ...
                print(
                    "ep {}: wrote {} transitions to group {} at process {} with {} finished".format(
                        num_processed,
        ...
        try:
            ...
            if args.real:
                ...
                for i in range(traj_len):
                    obs.append({k: np.array(obs_grp[k][i]) for k in obs_grp})
