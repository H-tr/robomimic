...
for model_name in model_config_mapping:
    ...
    inference_datasets_mapping = {
        "training": training_sampled_data
    }  # , "validation": validation_sampled_data}

    if action_names == None:
        # TODO
        action_keys = (
            config.train.action_keys
        )  # Need to adjust. For Robomimic datasets, there is no `action_keys`, it is config.train.dataset_keys
        modified_action_keys = [
            element.replace("action/", "") for element in action_keys
        ]
        action_names = []
        for i, action_key in enumerate(action_keys):
            if isinstance(
                training_sampled_data[0].__getitem__(0)[action_key][frame_stack - 1],
                np.ndarray,
            ):
                action_names.extend(
                    [
                        f"{modified_action_keys[i]}_{j+1}"
                        for j in range(
                            len(
                                training_sampled_data[0].__getitem__(0)[action_key][
                                    frame_stack - 1
                                ]
                            )
                        )
    ...
    for inference_key in inference_datasets_mapping:
        ...
        for d in inference_datasets_mapping[inference_key]:
            ...
            image_keys = [
                item for item in d.__getitem__(0)["obs"].keys() if "image" in item
            ]
            ...
            for batch in iter(dataloader):
                ...
                batch = model.postprocess_batch_for_training(
                    batch, obs_normalization_stats=None
                )  # ignore obs_normalization for now
                # model_output = model.nets["policy"](batch["obs"])

                model_output = model.get_action(batch["obs"])

                actual_action = TensorUtils.to_numpy(batch["actions"][0][0])
                predicted_action = TensorUtils.to_numpy(model_output[0])
            ...
            fig, axs = plt.subplots(
                len(images) + action_dim,
                1,
                figsize=(30, (len(images) + action_dim) * 3),
            )
            ...
            plt.subplots_adjust(
                left=0.05, right=0.95, top=0.95, bottom=0.05, wspace=0.3, hspace=0.6
            )

            # Save inference figures
            save_path = (
                saving_folder + inference_key + "/"
        ...
        step_log = {}
        step_log[f"{inference_key}_action_mse_error"] = (
            mse.mean().item()
        )  # average MSE across all timesteps averaged across all action dimensions (D,)
        ...
        for accuracy_threshold in accuracy_thresholds:
            step_log[f"{inference_key}_action_accuracy@{accuracy_threshold}"] = (
                torch.less(mse, accuracy_threshold).float().mean().item()
            )
        ...
        list_str = "\n".join(
            [
                "{} {}".format(desc, " ".join(map(str, sublist)))
                for desc, sublist in zip(data_name, mse_training_per_traj)
            ]
        )
        ...
        with open(txt_path, "w+") as txt_file:
            ...
            txt_file.write("\n")
            txt_file.write(
                f"Average MSE across trajectories per dimension: {average_mse_per_dimension}\n"
            )
            txt_file.write("\n")
