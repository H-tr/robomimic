...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.action_keys is not None:
            self.dataset_keys = tuple(
                set(self.dataset_keys).union(set(self.action_keys))
            )
    ...
    def get_action_traj(self, ep):
        ...
        for key in self.action_keys:
            action_traj[key] = self.hdf5_file["data/{}/{}".format(ep, key)][()].astype(
                "float32"
            )
        return action_traj
                ...
                for o1, o2 in zip(g1, g2):
                    for otype in ["obs", "next_obs", "goal_obs"]:
                        ...
                        if o1 not in output[otype] or o2 not in output[otype]:
                            continue
                        # swap values
                        output[otype][o1], output[otype][o2] = (
                            output[otype][o2],
                            output[otype][o1],
                        )
    ...
    def get_item(self, index):
        ...
        ac_dict = ObsUtils.normalize_dict(
            ac_dict, normalization_stats=action_normalization_stats
        )
...
class R2D2Dataset(SequenceDataset):
    ...
    def load_dataset_in_memory(
        ...
        for ep in LogUtils.custom_tqdm(demo_list):
            ...
            all_data[ep]["attrs"]["num_samples"] = hdf5_file[
                ...
                0
            ]  # hack to get traj len
            # get obs
            all_data[ep]["obs"] = {
                k: hdf5_file["observation/{}".format(k)][()].astype("float32")
                for k in obs_keys
            }
            if load_next_obs:
    ...
    def get_dataset_for_ep(self, ep, key, try_to_use_cache=True):
        ...
        # check if this key should be in memory
        key_should_be_in_memory = try_to_use_cache and (
            self.hdf5_cache_mode in ["all", "low_dim"]
        )
        if key_should_be_in_memory:
            # if key is an observation, it may not be in memory
            if "/" in key:
...
class MetaDataset(torch.utils.data.Dataset):
    ...
    def get_dataset_sampler(self):
        ...
        for i, (start, end) in enumerate(
            zip(self._ds_ind_bins[:-1], self._ds_ind_bins[1:])
        ):
    ...
    def get_action_stats(self):
        ...
        for dataset in self.datasets[1:]:
            ...
            meta_action_stats = _aggregate_traj_stats(
                meta_action_stats, ds_action_stats
            )
...
def _compute_traj_stats(traj_obs_dict):
    ...
    for k in traj_obs_dict:
        ...
        traj_stats[k]["sqdiff"] = ((traj_obs_dict[k] - traj_stats[k]["mean"]) ** 2).sum(
    ...
    for action_key in action_stats.keys():
        ...
        if norm_method is None:
            ...
            action_normalization_stats[action_key] = {
                "scale": np.ones_like(
                    action_stats[action_key]["mean"], dtype=np.float32
                ),
            ...
            action_normalization_stats[action_key] = {"scale": scale, "offset": offset}
            ...
            raise NotImplementedError(
                'action_config.actions.normalization: "{}" is not supported'.format(
                    norm_method
                )
            )
